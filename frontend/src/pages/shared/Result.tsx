...
function gradeColor(grade: string) {
  ...
  if (grade.startsWith('B')) return 'text-indigo-600 dark:text-indigo-400'
      ...
      {analytics && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            { label: 'Grade A+',       value: analytics.gradeDist?.['A+'] ?? 0, color: 'text-pink-400' },
          ].map(s => (
...
function TeacherResults() {
  ...
  const studentsQ = useQuery({ queryKey: ['adminStudents'], queryFn: async () => (await http.get('/admin/students')).data as { students: StudentUser[] } })
  ...
  const addMarksMut = useMutation({
    mutationFn: async () => (await http.post('/marks/add', { examId, courseId, studentUserId: studentId, marks: Number(marks), maxMarks: Number(maxMarks) })).data,
    onSuccess: (d) => {
      qc.invalidateQueries({ queryKey: ['pendingMarks'] })
      setSuccess(d.message ?? 'Marks saved!'); setError(''); setMarks(''); setStudentId('')
        ...
        <CardBody className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label className="grid gap-1">
              ...
              <input type="number" min={0} max={Number(maxMarks)} value={marks} onChange={e => setMarks(e.target.value)} placeholder="e.g. 85"
                className="h-10 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500" />
            </label>
            <label className="grid gap-1">
            ...
            <div className="flex items-end">
              <button type="button" disabled={!examId || !courseId || !studentId || !marks || addMarksMut.isPending}
